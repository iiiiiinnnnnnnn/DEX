...
HRESULT GpuResourceUtils::ConvertToDDS(const wchar_t* inputFilePath, const wchar_t* outputFilePath)
{
	DirectX::ScratchImage image;
	HRESULT hr = DirectX::LoadFromWICFile(inputFilePath, DirectX::WIC_FLAGS_NONE, nullptr, image);
	DirectX::ScratchImage mip;
	DirectX::GenerateMipMaps(image.GetImages(), image.GetImageCount(), image.GetMetadata(),
		DirectX::TEX_FILTER_DEFAULT, 0, mip);
	return DirectX::SaveToDDSFile(mip.GetImages(), mip.GetImageCount(), mip.GetMetadata(),
		DirectX::DDS_FLAGS_NONE, outputFilePath);
}
