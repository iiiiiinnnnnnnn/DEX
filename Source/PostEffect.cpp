...
// 開始処理
void PostEffect::Begin(Scene* scene)
{
...
// 輝度抽出処理
void PostEffect::LuminanceExtraction(Scene* scene, ID3D11ShaderResourceView* colorMap)
{
	ID3D11DeviceContext* dc = Graphics::Instance().GetDeviceContext();
...
// ブルーム処理
void PostEffect::Bloom(Scene* scene, ID3D11ShaderResourceView* colorMap,
...
// 終了処理
void PostEffect::End(Scene* scene)
{
	ID3D11DeviceContext* dc = Graphics::Instance().GetDeviceContext();
...
// デバッグGUI描画
void PostEffect::OnInspector()
{
	ImGui::SliderFloat("LuminanceLowerEdge", &cbPostEffect.luminanceExtractionLowerEdge, 0, 1.0f);
	ImGui::SliderFloat("LuminanceHigherEdge", &cbPostEffect.luminanceExtractionHigherEdge, 0, 1.0f);
	ImGui::SliderFloat("GaussianSigma", &cbPostEffect.gaussianSigma, 0, 10.0f);
	ImGui::SliderFloat("BloomIntensity", &cbPostEffect.bloomIntensity, 0, 10.0f);
}